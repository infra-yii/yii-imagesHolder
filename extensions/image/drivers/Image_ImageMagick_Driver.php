...
class Image_ImageMagick_Driver extends Image_Driver
{
    ...
    public function flip($dir)
    {
        ...
        $dir = ($dir === ImageProcessor::HORIZONTAL) ? '-flop' : '-flip';
    ...
    {
        switch ($prop['master']) {
            case ImageProcessor::WIDTH: // Wx
                $dim = escapeshellarg($prop['width'] . 'x');
                break;
            case ImageProcessor::HEIGHT: // xH
                $dim = escapeshellarg('x' . $prop['height']);
                break;
            case ImageProcessor::AUTO: // WxH
                $dim = escapeshellarg($prop['width'] . 'x' . $prop['height']);
                break;
            case ImageProcessor::NONE: // WxH!
                $dim = escapeshellarg($prop['width'] . 'x' . $prop['height'] . '!');
                break;
        }
