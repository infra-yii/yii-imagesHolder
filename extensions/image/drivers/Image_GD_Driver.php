...
class Image_GD_Driver extends Image_Driver
{
    ...
    public function flip($direction)
    {
        ...
        if ($direction === ImageProcessor::HORIZONTAL) {
            for ($x = 0; $x < $width; $x++) {
                $status = imagecopy($flipped, $this->tmp_image, $x, 0, $width - $x - 1, 0, 1, $height);
            }
        } elseif ($direction === ImageProcessor::VERTICAL) {
            for ($y = 0; $y < $height; $y++) {
    ...
    public function resize($properties)
    {
        ...
        if ($properties['master'] === ImageProcessor::AUTO) {
            // Change an automatic master dim to the correct type
            $properties['master'] = (($width / $properties['width']) > ($height / $properties['height'])) ? ImageProcessor::WIDTH : ImageProcessor::HEIGHT;
        }

        if (empty($properties['height']) OR $properties['master'] === ImageProcessor::WIDTH) {
            // Recalculate the height based on the width
            $properties['height'] = round($height * $properties['width'] / $width);
        }

        if (empty($properties['width']) OR $properties['master'] === ImageProcessor::HEIGHT) {
